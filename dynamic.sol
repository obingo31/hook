...
contract TakeProfitsHook is BaseHook, ERC1155 {
    ...
    mapping(PoolId poolId => mapping(address user => mapping(int24 tick => mapping(bool zeroForOne => int256 amount))))
    ...
    function placeOrder(PoolKey calldata key, int24 tick, uint256 amountIn, bool zeroForOne, Condition memory condition)
        external
        returns (uint256)
    {
    ...
    function cancelOrder(uint256 tokenId) external {
        ...
        address tokenToBeSoldContract =
            data.zeroForOne ? Currency.unwrap(data.poolKey.currency0) : Currency.unwrap(data.poolKey.currency1);
    ...
    function fillOrder(PoolKey calldata key, int24 tick, bool zeroForOne, int256 amountIn, uint256 tokenId) internal {
        ...
        if (destinationTokenId != 0 && tokenIdExists[destinationTokenId]) {
            ...
            placeOrder(
                destData.poolKey,
                destData.tick,
                amountOfTokensReceivedFromSwap / 2,
                destData.zeroForOne,
                orderConditions[destinationTokenId]
            );
        }
    }
    ...
    function getTokenId(PoolKey calldata key, int24 tickLower, bool zeroForOne, address user)
        public
        pure
        returns (uint256)
    {
